...
class GroupInvoices extends Component
{
    ...
    public $Group_id;
    public $price = 0;
    public $patient_id,$doctor_id,$section_id,$type,$catchError,$tax_value;
...
//----------------------------------------------------------------------------------------------------
    ...
    public function print($id)
    {
        ...
        $patient = Patient::findOrFail($single_invoice->patient_id); // تأكد من الحصول على المريض بناءً على id الفاتورة
        ...
        return Redirect::route('group_Print_single_invoices',[
            ...
            'patient_name' => $patient->name, // اسم المريض
            'patient_email' => $patient->email, // البريد الإلكتروني
            'patient_phone' => $patient->Phone, // رقم الهاتف
            'patient_gender' => $patient->Gender, // الجنس
            'patient_birth' => $patient->Date_Birth, // تاريخ الميلاد
            'patient_blood_group' => $patient->Blood_Group, // فصيلة الدم
            'id' => $single_invoice->id, // رقم الإيصال
            'bank_account' => $single_invoice->bank_account, // رقم الحساب البنكي (إذا كان متوفرًا)
            'bank_code' => $single_invoice->bank_code, // كود البنك (إذا كان متوفرًا)
        ]);
