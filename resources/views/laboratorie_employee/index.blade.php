...
@section('subhead')
    <title>{{ __('lab_employees.Employee List') }}</title>
    <style>
        .auto-width-select {
            width: auto;
            min-width: 120px;
...
@section('subcontent')
    ...
    <h2 class="intro-y text-lg font-medium mt-10">{{ __('lab_employees.Employee List') }}</h2>
    
    <div class="main-content" style="overflow-x: auto; min-height: 100vh;">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            <a href="{{ route('laboratorie_employee.create') }}">
        ...
        <div class="intro-y col-span-12 overflow-x-auto lg:overflow-visible">
            <table class="table table-report -mt-2 min-w-full">
                ...
                <tbody>
                    @foreach ($laboratorie_employees as $employee)
                    <tr class="intro-x">
                                ...
                                <div class="flex">
                                    <div class="w-10 h-10 image-fit zoom-in">
                                        @php
                                            ...
                                            $imagePath = $employeeImage ? asset('storage/properties/lab_employee/' . $employeeImage->filename) : asset('default-image.jpg');
                                        @endphp
                            ...
                            <td class="w-40">
                                <form action="{{ route('lab_EmployeeUpdateStatus', $employee->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                            ...
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    <a class="flex items-center mr-3" href="{{ route('laboratorie_employee.edit', $employee->id) }}">
                                        <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> {{ __('employees.Edit') }}
                                    </a>
                                    <a class="flex items-center text-danger" href="javascript:;" onclick="deleteEmployee('{{ route('laboratorie_employee.destroy', $employee->id) }}')">
