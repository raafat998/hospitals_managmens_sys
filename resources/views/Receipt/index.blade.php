...
@section('subhead')
    <title>{{ __('Receipt Accounts.Receipt List') }}</title>
@endsection
...
<h2 class="intro-y text-lg font-medium mt-10">{{ __('Receipt Accounts.Receipt List') }}</h2>

    <div class="main-content" style="overflow-x: auto; min-height: 100vh;">
    <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
        <a href="{{ route('Receipt.create') }}">
            <button class="btn btn-primary shadow-md mr-2">{{ __('Receipt Accounts.Add New Receipt') }}</button>
        </a>
    ...
    <div class="intro-y col-span-12 overflow-x-auto lg:overflow-visible">
        <table class="table table-report -mt-2 min-w-full">
            <thead>
                <tr>
                    <th class="whitespace-nowrap">{{ __('Receipt Accounts.ID') }}</th>
                    <th class="whitespace-nowrap">{{ __('Receipt Accounts.Patient ID') }}</th>
                    <th class="whitespace-nowrap">{{ __('Receipt Accounts.amount Amount') }}</th>
                    <th class="whitespace-nowrap">{{ __('Receipt Accounts.Description') }}</th>
                    <th class="text-center whitespace-nowrap">{{ __('Receipt Accounts.Actions') }}</th>
                </tr>
            </thead>
            <tbody>
                @foreach ($receipts as $receipt)
                    <tr class="intro-x">
                        <td class="w-40">{{ $receipt->id }}</td>
                        <td>{{ $receipt->patient_id }}</td>
                        <td>{{ $receipt->amount }}</td>
                        <td>{{ $receipt->description }}</td>
                        <td class="table-report__action w-56">
                            <div class="flex justify-center items-center">
                                <a class="flex items-center mr-3" href="{{ route('Receipt.show', $receipt->id) }}">
                                    <i data-lucide="Printer" class="w-4 h-4 mr-1"></i> {{ __('print') }}
                                </a>
                                <!-- Edit Button -->
                                <a class="flex items-center mr-3" href="{{ route('Receipt.edit', $receipt->id) }}">
                                    <i data-lucide="check-square" class="w-4 h-4 mr-1"></i> {{ __('Edit') }}
                                </a>
                                <!-- Delete Button -->
                                <form action="{{ route('Receipt.destroy', $receipt->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <a class="flex items-center text-danger" href="javascript:void(0);" onclick="deleteReceipt('{{ route('Receipt.destroy', $receipt->id) }}')">
...
<script>
    function deleteReceipt(actionUrl) {
        Swal.fire({
            title: 'Are you sure?',
