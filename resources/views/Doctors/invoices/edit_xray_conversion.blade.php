<!-- BEGIN: Modal Content -->
<div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="edit_xray_conversion{{$patient_ray->id}}" class="modal bg-black/60 w-screen h-screen fixed inset-0 flex items-center justify-center" style="display: none;">
        ...
        <form action="{{route('rays.update',$patient_ray->id)}}" method="POST">
            ...
            <div class="p-5">
                ...
                <div class="form-group mb-4">
                    ...
                    <textarea id="medicine" name="description" rows="6" class="form-control w-full h-24 border rounded-md shadow-sm focus:ring-4 focus:ring-primary dark:bg-darkmode-800 dark:border-darkmode-400" required>{{$patient_ray->description}}</textarea>
                </div>
            </div>
            <!-- END: Modal Body -->
